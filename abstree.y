...
%token NILL DEQNILL NEQNILL STRVAL CLASS ENDCLASS EXTENDS NEW THIS
...
%%
program : TypeDefBlock ClassDefBlock gdeclaration fdeflist mainblock {
                			                                            fclose(intermediate);
                			                                            exit(1);
                                        								}
        | TypeDefBlock ClassDefBlock gdeclaration mainblock    {
            		                                            fclose(intermediate);
            		                                            exit(1);
                                                			 }
        ;
...
ClassDecl : TypeName ID '(' ParamList ')' '{' LdeclBlock Body '}' {
                                                                    ...
                                                                    CFuncInstall(fbind,$2 -> name,Phead); 
            ...
            | TypeName ID ';'                                     {
                                                                    CVarInstall($2 -> name, declarationType);
                                                                }
            ;
...
Gidlist : ID            {
                            if(is_classtype)
                            	CGInstall($1 -> name,CdeclarationType,1,NULL);                                
                            else
                            {
                             verify($1,1,0,0,NULL);
        ...
        |Gidlist ',' ID   {
                                if(is_classtype)
                                 	CGInstall($3 -> name,CdeclarationType,1,NULL);                                
                                else
                                {
                                    verify($3,1,0,0,NULL);
...
TypeName :     INT  {declarationType = TLookup("integer");}
            ...
            |   ID {
                        ...
                        if(declarationType == NULL)
                        {
                            CdeclarationType = CLookup($1 -> name);
                            is_classtype = 1;
...
Stmt :  ID ASGN Expr ';'         {
        ...
        | ID ASGN ALLOC '(' ID ')' ';' {
                                        $1 -> type = NULL;
                                        $1 -> ctype = CLookup($5 -> name);
                                        $1 -> Gentry = GLookup($1 -> name);
                                        $5 -> type = NULL;

                                        head1 = TreeCreate(TLookup("integer"),NODE_ALLOC,NULL,NULL,NULL,NULL,NULL,NULL);
                                        head = TreeCreate(TLookup("integer"),NODE_NEW,NULL,NULL,NULL,head1,$5,NULL);
                                        $$ = TreeCreate(TLookup("void"), NODE_ASGN,NULL,NULL,NULL,$1,head,NULL);
                                     }
        | THIS '.' ID ASGN Expr ';' {
                                        cvtemp = CVLookup($3 -> name,CVhead);
                                        $3 -> type = cvtemp -> type;
                                        $3 -> value.intval = cvtemp -> ClassVarIndex; 
                                        head = TreeCreate(TLookup("void"),NODE_THIS,NULL,NULL,NULL,$3,NULL,NULL);
                                        $$ = TreeCreate(TLookup("void"), NODE_ASGN,NULL,NULL,NULL,head,$5,NULL);
                                    }
        | ID ASGN THIS '.' ID ';' {

                                        cvtemp = CVLookup($5 -> name,CVhead);
                                        $5 -> type = cvtemp -> type;
                                        $5 -> value.intval = cvtemp -> ClassVarIndex; 
                                        head = TreeCreate(TLookup("void"),NODE_THIS,NULL,NULL,NULL,$5,NULL,NULL);
                                        $$ = TreeCreate(TLookup("void"), NODE_ASGN,NULL,NULL,NULL,$1,head,NULL);
                                    }
        | THIS '.' ID ASGN THIS '.' ID ';' {
                                            ...
                                            cvtemp = CVLookup($7 -> name,CVhead);
                                            $7 -> type = cvtemp -> type;
                                            $7 -> value.intval = cvtemp -> ClassVarIndex; 
                                            
                                            head1 = TreeCreate(TLookup("void"),NODE_THIS,NULL,NULL,NULL,$7,NULL,NULL);
                                            
                                            $$ = TreeCreate(TLookup("void"), NODE_ASGN,NULL,NULL,NULL,head,head1,NULL);
                                        }
        | READ '(' THIS '.' ID ')' ';' {
                                        ...
                                        $5 -> value.intval = cvtemp -> ClassVarIndex; 
                                        head = TreeCreate(TLookup("void"),NODE_THIS,NULL,NULL,NULL,$5,NULL,NULL);
                                        $$ = TreeCreate(TLookup("void"), NODE_READ,NULL,NULL,NULL,NULL,head,NULL); 
                                    }
        | WRITE '(' THIS '.' ID ')' ';' {
                                        ...
                                        $5 -> value.intval = cvtemp -> ClassVarIndex; 
                                        head = TreeCreate(TLookup("void"),NODE_THIS,NULL,NULL,NULL,$5,NULL,NULL);
                                        $$ = TreeCreate(TLookup("void"), NODE_WRITE,NULL,NULL,NULL,NULL,head,NULL);
                                    }        
        ;
...
FIELD     : ID '.' ID   {
          ...
          | FIELD '.' ID {
                                ...
                                && temp1 != TLookup("array_integer") && temp1 != TLookup("array_string"))
                            {
                                if(temp1 == NULL)
                                {
                                    cvtemp = CVLookup($3 -> name,head -> ctype -> CVars);
                                    if(cvtemp != NULL)
                                    {
                                        $3 -> type = cvtemp -> type;
                                        head -> ptr2 = $3;
...
Expr : Expr PLUS Expr       {
        ...
        |ID '.' ID '(' ')' {
                                ...
                                $$ = TreeCreate(TLookup("integer"),NODE_CLASS_FUNC,NULL,NULL,NULL,$1,$3,NULL);
                            }
        |ID '.' ID '(' exprlist ')' {
                                    ...
                                    $1 -> ctype = $1 -> Gentry -> ctype;
		 							...
		 							cftemp1 = CFPcountLookup($3 -> name, Gtemp -> ctype -> CFuncs,exprcount);
                                    exprcount = 0;
		 							if(cftemp1 == NULL)
                                    {
                                    ...
                                    $5 -> ptr3 = cftemp1 -> paramlist;

                                    if(indicator == 1) //differentiating b/w one and more than one arguments
                                    {
                                        cindicator.intval = 1;
                                        $$ = TreeCreate(TLookup("integer"),NODE_CLASS_FUNC,NULL,&cindicator,NULL,$1,$3,$5);
                                        cindicator.intval = 0;
                                        indicator = 0;
                                    }
                                    else
        ...
        |THIS '.' ID '(' ')' {
                                cftemp1 = CFLookup($3 -> name, CFhead);
                                if(cftemp1 == NULL)
                                {
                                    yyerror("cannot find the function with in the class\n");
                                }
                                $$ = TreeCreate(TLookup("integer"),NODE_FUNC,$3 -> name,NULL,cftemp1 -> paramlist,NULL,NULL,NULL);
                            }
        |THIS '.' ID '(' exprlist ')' {
                                        cftemp1 = CFPcountLookup($3 -> name, CFhead,exprcount);
                                        exprcount = 0;
                                        if(cftemp1 == NULL)
                                        {
                                            yyerror("No such function in the class\n");
                                        ...
                                        if(indicator == 1) //differentiating b/w one and more than one arguments
                                        {
                                            $$ = TreeCreate(TLookup("integer"),NODE_FUNC,$3 -> name,&cindicator,cftemp1 -> paramlist,NULL,$5,NULL);
                                            indicator = 0;
                                        }
                                        else
                                            $$ = TreeCreate(TLookup("integer"),NODE_FUNC,$3 -> name,&cindicator,cftemp1 -> paramlist,NULL,NULL,$5);
